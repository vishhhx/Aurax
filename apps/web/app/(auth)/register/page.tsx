...
export default function RegisterPage() {
  ...
  const handleOAuthRedirect = async (provider: "google" | "github") => {
    ...
    setErrorMsg(null)
    try {
      // validateStatus: accept anything below 500 so axios resolves even on HTTP 301
      ...
      const message =
        ...
        err.message ||
        `Failed to initiate ${provider === "google" ? "Google" : "GitHub"} sign-up. Please try again.`
      setErrorMsg(message)
    }
  }
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setErrorMsg("Email/password registration is not yet enabled. Please use Google or GitHub.")
    } finally {
      setIsLoading(false)
    }
  }
    ...
    <div className="w-full max-w-md">
      <div className="space-y-8">
        ...
        <div className="space-y-3 text-center">
          ...
          <p className="text-sm text-muted-foreground">Join Aurax to start trading securely</p>
        </div>
        ...
        <div className="space-y-3">
          <Button
            ...
            type="button"
            variant="outline"
            className="h-11 w-full gap-3 border-border/60 bg-card/40 hover:bg-muted/60 transition-all"
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <div className="flex items-start gap-2">
            <Checkbox
              id="terms" name="terms" checked={formData.terms}
              onCheckedChange={(checked) => setFormData((prev) => ({ ...prev, terms: checked === true }))}
              disabled={isLoading} className="mt-1"
            />
            <Label htmlFor="terms" className="text-xs text-muted-foreground cursor-pointer font-normal leading-relaxed">
              I agree to the{" "}
          ...
          <Button
            ...
            type="submit" className="w-full h-10 mt-2"
            disabled={isLoading || oauthLoading !== null || !formData.terms}
          >
            {isLoading ? (
